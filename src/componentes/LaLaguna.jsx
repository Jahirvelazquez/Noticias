...
const LaLaguna = () => {
    const [lagunaNews, setLagunaNews] = useState([]);
    const [loading, setLoading] = useState(true);
    const [currentPage, setCurrentPage] = useState(1);
    const newsPerPage = 10; // Número de noticias por página

    useEffect(() => {
        const lagunaNewsRef = ref(database, 'news/');
        onValue(lagunaNewsRef, (snapshot) => {
    ...
    const totalPages = Math.ceil(lagunaNews.length / newsPerPage);
    ...
    const currentNews = lagunaNews.slice(indexOfFirstNews, indexOfLastNews);
            ...
            <div className="header-container">
                <div className="header-content">
                    <div className="header-padding">
                        <h1 className="header-title">CATEGORÍA: La Laguna</h1>
                    </div>
                </div>
            </div>
                ...
                <Link to={`/news/${item.id}`} key={item.id} className="news-item-link">
                    <div className="news-item">
                        ...
                        {item.fileUrls && item.fileUrls.length > 0 && (
                            <img src={item.fileUrls[0]} alt="La Laguna News" className="news-image" />
                        )}

                        {item.videoUrls && item.videoUrls.length > 0 && (
                            <iframe
                                src={item.videoUrls[0]}
                                title="Video de Noticia LA LAGUNA"
                                allowFullScreen
