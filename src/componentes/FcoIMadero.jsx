...
const FcoLMadero = () => {
  const [maderoNews, setMaderoNews] = useState([]);
    ...
    onValue(newsRef, (snapshot) => {
      ...
      const newsArray = data
        ? Object.entries(data)
            ...
            .filter(
              (item) =>
                item.category === 'Fco.l.Madero' ||
                (item.category && item.category.includes('Fco.l.Madero'))
            )
        : [];
      setMaderoNews(newsArray.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime)));
      setLoading(false);
    });
  }, []);

  const totalPages = Math.ceil(maderoNews.length / newsPerPage);
  ...
  const currentNews = maderoNews.slice(indexOfFirstNews, indexOfLastNews);
      ...
      <div className="header-container">
        <div className="header-content">
          <div className="header-padding">
            <h1 className="header-title">CATEGORÍA: Fco.L.Madero</h1>
          </div>
        </div>
      </div>
              ...
              <div className="news-card">
                <MDBCard className='mb-3'>
                  <MDBRow className='g-0'>
                    ...
                    <MDBCol md='7'>
                      <MDBCardBody>
                        <MDBBadge color='primary' pill>Fco.L.Madero</MDBBadge>
