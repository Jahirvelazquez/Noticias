...
const AdminCarrusel = () => {
  ...
  const handleUpload = async (e) => {
    ...
    for (let file of files) {
      const fileRef = storageRef(storage, `carrusel/${newsId}_${file.name}`);
      await uploadBytes(fileRef, file);
      const fileUrl = await getDownloadURL(fileRef);
      publicidadUrls.push(fileUrl);
    }

    await set(ref(database, "carrusel/" + newsId), {
  ...
  const handleDelete = async (id, urls) => {
    ...
    await remove(ref(database, "carrusel/" + id));
    alert("carrusek eliminada.");
  };

  return (
    <>
      <AdminNavbar />
      <h1>Subir Publicidad Carrusel</h1>
          ...
          <div key={publi.id} style={cardStyle}>
            {publi.fileUrls.map((url, idx) =>
              url.includes("video") ? (
                ...
                <img key={idx} src={url} alt="Carrusel" style={mediaStyle} />
              )
            )}
            <button
